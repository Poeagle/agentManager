...
export class VirtualTerminal {
  ...
  /**
   * @param faithful When true, feed PTY data verbatim (honor cursor positioning,
   *   clears, and the alt-screen buffer) so `getScreen()` returns the ACTUAL
   *   current screen. Required for detecting interactive prompts in full-screen
   *   TUIs — the default append-only mode flattens redraw frames into a pile and
   *   corrupts the visible screen. When false (default), strip backward movement
   *   so redraws append (scrollback-style capture).
   */
  constructor(cols = 120, rows = 40, faithful = false) {
    this._faithful = faithful;
    this.term = new Terminal({
      cols,
      rows,
      scrollback: faithful ? 2000 : 50000,
  ...
  write(data: string): void {
    // In faithful mode, write verbatim so the emulator reflects the true screen.
    // Otherwise strip sequences that cause backward movement / overwrites.
    const cleaned = this._faithful ? data : data.replace(STRIP_RE, '');
    if (!cleaned) return;
