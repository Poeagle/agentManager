...
// clean up hooks written before this rename (octoally → agentmanager).
const LEGACY_HOOK_MARKERS = ['# octoally-events-hook'];
const HOOK_MARKERS = [AGENTMANAGER_HOOK_MARKER, ...LEGACY_HOOK_MARKERS];

/**
 * Build the inline hook command that POSTs tool use events to AgentManager.
...
function isHookInstalled(settings: ClaudeSettings): boolean {
  ...
  return entries.some((entry) =>
    entry.hooks?.some((h) => HOOK_MARKERS.some((m) => h.command?.includes(m)))
  );
}
...
function installHook(settings: ClaudeSettings, projectPath: string): ClaudeSettings {
  ...
  // Replace any existing AgentManager/HiveCommand hook so installs upgrade stale commands.
...
function uninstallHook(settings: ClaudeSettings): ClaudeSettings {
  ...
  settings.hooks.PostToolUse = settings.hooks.PostToolUse.filter(
    (entry) => !entry.hooks?.some((h) => HOOK_MARKERS.some((m) => h.command?.includes(m)))
  );
...
export const hooksRoutes: FastifyPluginAsync = async (app) => {
  ...
  // Install or uninstall the AgentManager events hook
  app.post<{
