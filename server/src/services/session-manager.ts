...
/** Check if a tmux session is alive on the tmux server */
...
/** List all AgentManager tmux session IDs that are still alive (checks legacy servers too) */
function tmuxListAgentmanagerSessionIds(): string[] {
...
const DTACH_PREFIXES = ['agentmanager-'];
...
/** List all AgentManager dtach sessions that are still alive (checks legacy prefixes too) */
function dtachListAgentmanagerSessions(): string[] {
  const ids = new Set<string>();
  try {
...
function wireWorker(sessionId: string, worker: ChildProcess, projectPath?: string, preSpawnFiles?: Set<string>): ActiveSession {
  ...
  // Broadcast live process-state changes to the dashboard stream so session tabs
  // can show real-time signal lights (busy / waiting / idle). Wire once per
  // tracker instance (getOrCreateTracker may return an existing one on reconnect).
  ...
  worker.on('message', (msg: any) => {
    switch (msg.type) {
      ...
      case 'exit': {
        ...
        pushSystemEvent(`[AgentManager] Session ${sessionId} ${status} (exit ${msg.exitCode}): ${taskSnippet}`);
...
/**
 * Release an AgentManager session back to its dtach socket without killing the process.
 * Used by pop-out: tears down the AgentManager worker/tmux wrapper but leaves the
 ...
 * For regular sessions, returns the AgentManager dtach socket path.
 */
...
// an AgentManager tmux PTY, which incorrectly killed: (1) adopted external sessions
...
// handles dead AgentManager sessions individually via killOrphanedProcess.
...
export async function cleanupStaleRunningSessions(): Promise<void> {
  ...
  if (config.useDtach || config.useTmux) {
    ...
    const t1 = Date.now();
    const aliveDtach = config.useDtach ? new Set(dtachListAgentmanagerSessions()) : new Set<string>();
    const aliveTmux = config.useTmux ? new Set(tmuxListAgentmanagerSessionIds()) : new Set<string>();
  ...
  // That function killed ANY claude process not on an AgentManager tmux PTY,
  ...
  // already handles dead AgentManager sessions individually.
 ...
 */
export function purgeSessionRecord(sessionId: string): { ok: boolean; error?: string } {
  const db = getDb();
  const session = getSession(sessionId);
  if (!session) return { ok: false, error: 'Session not found' };
  if (activeSessions.has(sessionId) || ['running', 'detached', 'pending'].includes(session.status)) {
  ...
  // Remove the Claude JSONL conversation file, if we can locate it.
  try {
...
async function isAgentManagerOwnedAsync(pid: number): Promise<boolean> {
  try {
    const environ = await readFileAsync(`/proc/${pid}/environ`, 'utf8');
    return environ.includes('AGENTMANAGER_SESSION=');
  } catch {
    return false;
  }
}
...
export async function discoverExternalSessions(projectPath?: string): Promise<DiscoverableSession[]> {
  ...
  try {
    ...
    for (const f of files) {
      ...
      const ownerChecks = await Promise.all(hivePids.map(p => isAgentManagerOwnedAsync(p)));
  ...
  // Also discover released AgentManager tmux sessions (popped-out sessions)
...
export async function adoptDtachSession(socketPath: string, projectId?: string): Promise<Session | null> {
  ...
  const ownerChecks = await Promise.all(socketPids.map(p => isAgentManagerOwnedAsync(p)));
...
/**
 * Re-adopt a released AgentManager tmux session (popped-out and now being brought back).
...
async function readoptReleasedSession(tmuxName: string, _projectId?: string): Promise<Session | null> {
  ...
  // so they don't fight with AgentManager's worker for input/output
  try {
