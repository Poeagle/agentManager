...
export interface SessionState {
  ...
  choices: string[] | null;
  /** True when the detected prompt is a tool-permission / authorization request
      (Claude Code "Do you want to proceed?", "don't ask again", etc.) — drives
      the 🔒 variant of the yellow "needs you" tab signal. */
...
const CONFIRM_PATTERN = /\((?:y\/n|n\/y|yes\/no|no\/yes)\)\s*$/i;
const TEXT_PROMPT_PATTERN = /(?:>\s*$|\?\s*$)/;

// Tool-permission / authorization prompts (Claude Code & Codex) — distinguishes
// an "authorize?" prompt (🔒) from an ordinary choice menu (e.g. /model picker).
const PERMISSION_PATTERN = /(do you want to (proceed|allow|continue|run|make|create|apply|edit|read|write)|don'?t ask again|tell claude what to do|allow (this|command|the)|grant (permission|access)|permission to|run this command|approve this)/i;

// Selection-cursor glyphs used by Claude/Codex TUI menus (the pointer on the
// currently-highlighted option). Plain ">" is excluded — it's the idle input box.
const CURSOR_PATTERN = /[❯➤►▶▸»]/;
// Leading TUI "gutter" before option text: box borders, pipes, the cursor,
// bullets, spaces. Stripped so "│ ❯ 1. Yes" exposes "1. Yes".
const GUTTER_PATTERN = /^[\s│┃|‖╎╏┆┇┊┋>*\-•❯➤►▶▸»]+/;

function detectPrompt(text: string): { type: PromptType; choices: string[] | null; isPermission: boolean } {
  // Only look at the tail of output for prompt detection
  const tail = text.slice(-2000);
  const isPermission = PERMISSION_PATTERN.test(tail);

  // Numbered options — tolerant of TUI box borders / selection cursor. Claude
  // and Codex render permission/choice prompts as a boxed list:
  //   │ ❯ 1. Yes                                  │
  //   │   2. No, and tell Claude what to do (esc) │
  // We require options 1 AND 2, plus a selection cursor (or permission phrasing)
  // so a plain numbered list in assistant prose isn't mistaken for a prompt.
  let hasOne = false;
  let hasTwo = false;
  let hasCursor = false;
  const byNum = new Map<number, string>();
  for (const raw of tail.split('\n')) {
    if (CURSOR_PATTERN.test(raw)) hasCursor = true;
    const cleaned = raw.replace(GUTTER_PATTERN, '').trimEnd();
    const m = cleaned.match(/^(\d+)[.)]\s+(.+)$/);
    if (m) {
      const n = parseInt(m[1], 10);
      if (n === 1) hasOne = true;
      if (n === 2) hasTwo = true;
      // Strip the trailing box border ("Yes ... │") left by the right edge.
      const optText = m[2].replace(/[\s│┃|‖╎╏┆┇┊┋»]+$/, '').trim();
      if (!byNum.has(n)) byNum.set(n, optText);
    }
  }
  if (hasOne && hasTwo && (hasCursor || isPermission)) {
    const choices = [...byNum.entries()].sort((a, b) => a[0] - b[0]).map(([, t]) => t);
    return { type: 'choice', choices, isPermission };
  }

  if (CONFIRM_PATTERN.test(tail)) {
    return { type: 'confirmation', choices: ['Yes', 'No'], isPermission };
  }

  if (TEXT_PROMPT_PATTERN.test(tail)) {
    return { type: 'text', choices: null, isPermission };
  }
...
export class SessionStateTracker {
  ...
  private _isPermission: boolean = false;
  ...
  // Faithful emulator (honors cursor positioning / clears / alt-buffer) so the
  // detected screen matches what the user actually sees — essential for
  // interactive prompts in full-screen TUIs.
  private _vt: VirtualTerminal = new VirtualTerminal(120, 40, true);
  ...
  get state(): SessionState {
    return {
      ...
      isPermission: this._isPermission,
    };
  }
  ...
  private _onQuiescence(): void {
    ...
    const { type, choices, isPermission } = detectPrompt(screen || this._outputSinceInput);

    if (type) {
      this._promptType = type;
      this._choices = choices;
      this._isPermission = isPermission;
