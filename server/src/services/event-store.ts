...
/**
 * Broadcast an event to live stream listeners WITHOUT persisting it to the DB.
 * Use for high-frequency, ephemeral signals (e.g. live session process-state)
 * that should drive the UI in real time but don't belong in the event history.
 * The synthetic event carries `project_id` so the stream's per-user ownership
 * filter still applies.
 */
export function broadcastEphemeral(input: EventInput): void {
  notifyListeners({
    id: -1,
    session_id: input.session_id ?? null,
    project_id: (input.project_id ?? null) as any,
    type: input.type,
